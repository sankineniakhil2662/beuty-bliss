...
// Exported so the client can mint the ref up front (BookingWizard's submit
// handler) and pass it through — keeps the WhatsApp message, the success
// screen, and the Firestore record all showing the same ref.
export function makeRef(year) {
...
//            consultation:{...}, preferredDate:"YYYY-MM-DD", ref? }
// `ref` is optional — reuses a client-minted ref if provided, else mints one.
// Returns { id, ref }.
export async function createBooking(payload) {
  const year = (payload.preferredDate || "").slice(0, 4) || String(new Date().getFullYear());
  const ref = payload.ref || makeRef(year);
