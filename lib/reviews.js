import {
  addDoc,
  collection,
  deleteDoc,
  doc,
  getDocs,
  orderBy,
  query,
  serverTimestamp,
  Timestamp,
  updateDoc,
  where,
} from "firebase/firestore";
import { db } from "./firebase";
import { withTimeout } from "./withTimeout";

// TEMP — Facebook review migration only. Re-anchors a picked Date (built
// from local y/m/d, e.g. by TempReviewDatePicker) to UTC midnight so that
// mapReview()'s `.toDate().toISOString().slice(0,10)` always reads back the
// exact same y/m/d, regardless of the browser's or server's timezone.
// Remove alongside the `reviewDate` branch in submitReview() below once
// migration is complete.
function toUtcMidnightTimestamp(date) {
  return Timestamp.fromMillis(
    Date.UTC(date.getFullYear(), date.getMonth(), date.getDate())
  );
}

// Canonical review data (Firestore shape). Mirrors the mockup's sample reviews
// and is used by scripts/seed-reviews.mjs.
export const SEED_REVIEWS = [
  {
    name: "Aisha K.",
    service: "Korean Glass Facial",
    rating: 5,
    text: "My skin has never looked this dewy. Sruthi explained every step and the follow-up was so personal.",
    photos: ["placeholder", "placeholder"],
    status: "approved",
  },
  {
    name: "Megan R.",
    service: "Microneedling",
    rating: 5,
    text: "Saw a real difference in my acne scars after just two sessions. Clean studio, gentle hands.",
    photos: ["placeholder"],
    status: "approved",
  },
  {
    name: "Priya S.",
    service: "Pearl Facial",
    rating: 5,
    text: "Booked a day, got a call within the hour to confirm. Felt looked-after from start to finish.",
    photos: [],
    status: "approved",
  },
  {
    name: "Linda M.",
    service: "June Glow Combination",
    rating: 4,
    text: "Lovely combination facial and great value. Glow lasted well over a week.",
    photos: [],
    status: "approved",
  },
  {
    name: "Fatima A.",
    service: "Glutathione Needling",
    rating: 5,
    text: "My pigmentation has visibly faded. Honest advice, no upselling. Highly recommend.",
    photos: ["placeholder", "placeholder"],
    status: "approved",
  },
  {
    name: "Chloe T.",
    service: "Back Polish Massage",
    rating: 5,
    text: "So relaxing and my back felt brand new. Already booked my next visit.",
    photos: [],
    status: "approved",
  },
];

// Formats a review's `date` (a "YYYY-MM-DD" string — see mapReview) for
// display. Parsed and formatted in UTC throughout: a bare "YYYY-MM-DD" string
// is parsed by `Date` as UTC midnight, so formatting must also read it back
// in UTC — otherwise viewers behind UTC (most of the Americas) see the day
// before whatever was actually stored. Shared by ReviewCard and ReviewModal
// so a review always shows the same date in both places.
export function formatReviewDate(dateStr, options) {
  if (!dateStr) return null;
  return new Date(dateStr).toLocaleDateString("en-US", {
    month: "long",
    day: "numeric",
    year: "numeric",
    ...options,
    timeZone: "UTC",
  });
}

// Map a Firestore/canonical review into the shape ReviewCard consumes, keeping
// both field names: name/nm, service/svc, rating/r, text/t, photos.
export function mapReview(data) {
  return {
    id: data.id ?? null,
    name: data.name,
    nm: data.name,
    service: data.service,
    svc: data.service,
    rating: data.rating,
    r: data.rating,
    text: data.text,
    t: data.text,
    photos: data.photos && data.photos.length ? data.photos : null,
    status: data.status,
    date: data.createdAt?.toDate
      ? data.createdAt.toDate().toISOString().slice(0, 10)
      : null,
  };
}

// Read approved reviews, newest first.
// Constrained to status == "approved" (single-field filter, no composite index)
// so it satisfies the public read rule; sorted by createdAt in JS.
export async function getApprovedReviews() {
  const snap = await withTimeout(
    getDocs(query(collection(db, "reviews"), where("status", "==", "approved"))),
    7000,
    "Loading reviews"
  );
  return snap.docs
    .map((d) => ({ id: d.id, ...d.data() }))
    .sort((a, b) => (b.createdAt?.seconds ?? 0) - (a.createdAt?.seconds ?? 0))
    .map(mapReview);
}

// ---- Admin (authenticated as admin; gated by Firestore rules) ----

// All reviews for moderation, newest-first, mapped to the ReviewModeration
// table shape ({ id, name, service, rating, text, photos, ref, status, date }).
export async function fetchAllReviewsForAdmin() {
  const snap = await withTimeout(
    getDocs(query(collection(db, "reviews"), orderBy("createdAt", "desc"))),
    7000,
    "Loading reviews"
  );
  return snap.docs.map((d) => {
    const r = d.data();
    return {
      id: d.id,
      name: r.name,
      service: r.service,
      rating: r.rating,
      text: r.text,
      photos: r.photos && r.photos.length ? r.photos : null,
      ref: r.ref ?? r.bookingRef ?? "",
      status: r.status,
      date: r.createdAt?.toDate
        ? r.createdAt.toDate().toISOString().slice(0, 10)
        : "",
    };
  });
}

export async function setReviewStatus(id, status) {
  await updateDoc(doc(db, "reviews", id), { status });
}

// Public review submission → lands as "pending" and only shows after admin
// approval. (The secure /review/[token] flow in Phase 4 will add real client
// verification + Storage photo uploads on top of this.)
export async function submitReview({ name, service, rating, text, photos, reviewDate }) {
  await addDoc(collection(db, "reviews"), {
    name: (name || "").trim(),
    service: (service || "").trim(),
    rating: Number(rating) || 5,
    text: (text || "").trim(),
    photos: Array.isArray(photos) ? photos : [],
    status: "pending",
    // TEMP — Facebook review migration only: when the "Drop a review" popup's
    // temporary date field is used, back-date createdAt to the picked day
    // instead of "now" (see toUtcMidnightTimestamp above). Everything that
    // reads createdAt (display, sorting, admin moderation) needs no changes.
    // Remove this ternary (keep just `serverTimestamp()`) once migration is
    // complete.
    createdAt: reviewDate instanceof Date ? toUtcMidnightTimestamp(reviewDate) : serverTimestamp(),
  });
}

export async function removeReview(id) {
  await deleteDoc(doc(db, "reviews", id));
}
